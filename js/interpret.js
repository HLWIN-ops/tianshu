...
(function () {
  ...
  const USE_GOD_ADVICE = {
    '木': { 方位: '东方', 颜色: '青绿', 行业: '文化教育、农林、设计创意、出版', 数字: '3、8', 配饰: '绿植、木质、翡翠', 饮食: '酸味蔬果、养肝' },
    ...
    '水': { 方位: '北方', 颜色: '黑蓝', 行业: '贸易、物流、信息、旅游、航运', 数字: '1、6', 配饰: '流水意象、黑曜、玻璃', 饮食: '咸淡适中、补肾养精' },
  };
  ...
  const DOMAIN_RULES = {
    ...
    '财运': (r, pat, ug) => {
      ...
      if (pat.combos.includes('伤官生财') || pat.combos.includes('食神生财')) {
        s = '食伤生财：钱从才智/作品/技能来，宜活络经营与知识变现，忌死工资一条道。';
      } else if (hasZheng && hasPian) {
        s = '正偏财皆透：正职收入+副业/投资双轨，主稳辅活，设预算比例。';
      } else if (hasZheng) {
        s = '正财透干：宜工资、合同、固定项目与房产配置，少赌性投机。';
      } else if (hasPian) {
        s = '偏财透干：人脉与机会财明显，宜抓风口但严格止损。';
      } else {
        s = '财星不显：以技换酬、量入为出；待财运大运/流年再加码。';
      }
      if (hasBi) s += '比劫较显，合伙分清股权与流水，防争财破耗。';
      if (r.strength.level === '身弱' && (hasZheng || hasPian)) s += '身弱财重时勿同时多开项目，先立身再求财。';
      return s;
    },
    '婚姻感情': (r, pat, ug) => {
      const dayZhi = r.pillars.day.zhiName;
      ...
      dayRel.forEach(rr => {
        if (rr.type.includes('冲')) s += `日支见冲（${rr.az}${rr.bz}），婚姻宫有动荡象，宜晚婚或婚后多沟通。`;
        if (rr.type.includes('合') || rr.type.includes('半合')) s += '日支有合，姻缘易成亦易羁绊。';
        if (rr.type.includes('害') || rr.type.includes('刑')) s += '日支刑害，相处中易有误解，宜把话说明白。';
      });
      if (r.input.gender === 'male') {
        const hasCai = pat.tianShen.includes('正财') || pat.tianShen.includes('偏财');
        s += hasCai ? '男命财星现，妻缘有托。' : '男命财星隐，妻缘多待运年引动。';
      } else {
        const hasGuan = pat.tianShen.includes('正官') || pat.tianShen.includes('七杀');
        s += hasGuan ? '女命官杀现，夫缘可期。' : '女命官星隐，良缘多待运年逢合。';
      }
      const hasTao = (r.shensha || []).some(ss => ss.name === '桃花');
      if (hasTao) s += '命带桃花，人缘魅力足，择人宜慎、界线要清。';
      return s;
    },
    '健康': (r, pat, ug) => {
      ...
      const map = {
        '木': '肝胆筋骨', '火': '心脑血脉', '土': '脾胃肌肤', '金': '肺肠呼吸', '水': '肾系泌尿',
      };
      let s = `加权五行最旺 <span class="highlight">${strong}</span>（${(w[strong] || 0).toFixed(1)}，防${map[strong]}过亢），最弱 <span class="highlight">${weak}</span>（${(w[weak] || 0).toFixed(1)}，宜养${map[weak]}）。`;
      s += `喜用${ug.xi.map(wxTag).join('、')}可作起居色与向参考。`;
      if ((r.shensha || []).some(s => s.name === '天医')) s += '天医在命，留意养生亦易近医道医药信息。';
      if ((r.shensha || []).some(s => s.name === '羊刃')) s += '羊刃主刚，防运动损伤与血光，宜温和锻炼。';
      s += '健康以现代医学为准，此仅传统养生参考。';
      return s;
    },
    '学业': (r, pat, ug) => {
      const hasYin = pat.tianShen.includes('正印') || pat.tianShen.includes('偏印');
  ...
  function renderPracticalSection(r) {
    ...
    const xiList = ug.xi || [];
    const jiList = ug.ji || [];
    let html = '<div class="read-card"><h3>开运实务建议</h3>';
    html += '<p class="sub-note">由喜用忌神映射的日常参考，重在习惯，勿迷信符咒。</p>';
    ...
    html += item('方位环境', `多活动于 ${dirs.join('、')}；座位/床头可优先朝喜用方。忌神 ${jiList.map(wxTag).join('、')} 之方少久留。`);
    html += item('颜色衣饰', `衣着配饰偏 ${colors.join('、')}；可点缀 ${goods.join('、')}。`);
    html += item('事业方向', industries.join('；') + '。转行/副业可优先落在喜用五行行业。');
    html += item('数字择日', `常用吉数参考 ${nums.join('、')}（尾号、楼层等作心理暗示即可）。`);
    html += item('起居饮食', foods.join('；') + '。作息稳定比任何开运物更重要。');
